...
const USER_ENDPOINT = 'https://api.twitter.com/1.1/users/lookup.json?screen_name=';
const LIKES_ENDPOINT = 'https://api.twitter.com/1.1/favorites/list.json?count=200&screen_name=';
const TWEETS_ENDPOINT = 'https://api.twitter.com/1.1/statuses/user_timeline.json?count=200&screen_name=';
const USER_LOOKUP_ENDPOINT = 'https://api.twitter.com/1.1/users/lookup.json?user_id=';
...
export const handler: Handler = async (event) => {
 try {
   ...
   const userResponse = await axios.get(`${USER_ENDPOINT}${username}`, {headers,});
   const ownData = extractUserInfo(userResponse.data[0]);
   if (ownData.private) {
     return {statusCode: 200, body: JSON.stringify({ownData: {private: true,},}),};
   }
   ...
   usersData = usersData.concat(likes.map((tweet) => extractUserInfo(tweet.user)));
   ...
   const mentionedUserIds = _countBy(mentions.map((tweet) => tweet.in_reply_to_user_id_str));
